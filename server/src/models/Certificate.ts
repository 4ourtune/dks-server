...
class CertificateModel {
  ...
  async ensureRootCATable(): Promise<void> {
    ...
    await this.db.run(
      `CREATE INDEX IF NOT EXISTS idx_root_ca_keys_active ON root_ca_keys(is_active) WHERE is_active = true`,
    );
  }
  ...
  async findBySerialNumber(serialNumber: string): Promise<Certificate | null> {
    ...
    return result.rows.length > 0
      ? this.mapRowToCertificate(result.rows[0])
      : null;
  }
  ...
  private mapRowToCertificate(row: any): Certificate {
    ...
    const certificateData =
      typeof rawCertificateData === "string"
