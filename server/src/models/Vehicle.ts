...
type UpdateVehicleInput = Partial<
  Pick<
    Vehicle,
    "vin" | "model" | "owner_id" | "device_id" | "status" | "secret_hash"
  >
>;
...
class VehicleModel {
  ...
  async findById(id: number): Promise<Vehicle | null> {
    const vehicle = await this.db.get("SELECT * FROM vehicles WHERE id = $1", [
      id,
    ]);
    return vehicle || null;
  }
  ...
  async findByVin(vin: string): Promise<Vehicle | null> {
    const vehicle = await this.db.get("SELECT * FROM vehicles WHERE vin = $1", [
      vin,
    ]);
    return vehicle || null;
  }
  ...
  async delete(id: number): Promise<boolean> {
    const result = await this.db.run("DELETE FROM vehicles WHERE id = $1", [
      id,
    ]);
