...
class DigitalKeyModel {
  ...
  async create(
    keyData: Omit<DigitalKey, "id" | "created_at" | "updated_at">,
  ): Promise<DigitalKey> {
  ...
  async findById(id: number): Promise<DigitalKey | null> {
    const key = await this.db.get("SELECT * FROM digital_keys WHERE id = $1", [
      id,
    ]);
  ...
  async update(
    id: number,
    keyData: Partial<Omit<DigitalKey, "id" | "created_at" | "updated_at">>,
  ): Promise<DigitalKey | null> {
  ...
  async delete(id: number): Promise<boolean> {
    const result = await this.db.run("DELETE FROM digital_keys WHERE id = $1", [
      id,
    ]);
  ...
  async logAccess(
    logData: Omit<AccessLog, "id" | "timestamp">,
  ): Promise<AccessLog> {
    const result = await this.db.run(
