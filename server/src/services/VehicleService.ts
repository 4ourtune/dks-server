...
type VehicleRegistrationInput = Omit<
  Vehicle,
  "id" | "created_at" | "updated_at" | "secret_hash"
>;
...
class VehicleService {
  ...
  ): Promise<{ vehicle: Vehicle; secret: string }> {
    const existingVehicle = await this.vehicleModel.findByVin(vehicleData.vin);
    if (existingVehicle) {
      throw new Error("Vehicle with this VIN already exists");
    }

    const existingDevice = await this.vehicleModel.findByDeviceId(
  ...
  async setVehicleSecret(
    ...
    const updatedVehicle = await this.vehicleModel.updateSecretHash(
      vehicleId,
      secretHash,
    );
  ...
  async deleteVehicle(vehicleId: number): Promise<boolean> {
    ...
    if (hasActiveKeys) {
      throw new Error(
        "Cannot delete vehicle with active digital keys. Revoke all keys first.",
      );
    }
  ...
  async updateVehicleStatus(
    ...
    const currentStatus =
      this.connectedVehicles.get(vehicleId) || this.getDefaultVehicleStatus();
  ...
  async getVehicleStatistics(vehicleId: number): Promise<{
    ...
    const successfulAccess = logs.filter(
  ...
  async performMaintenanceCheck(vehicleId: number): Promise<{
    ...
    if (recentFailures > 5) {
      ...
      recommendations.push(
        "Check digital key permissions and vehicle connectivity",
      );
    }
    ...
    const maintenanceStatus =
      issues.length === 0
        ? "passed"
        : issues.some(
              (issue) => issue.includes("battery") || issue.includes("failure"),
            )
          ? "failed"
          : "warning";
  ...
  async setVehicleMaintenanceMode(
    vehicleId: number,
    enabled: boolean,
  ): Promise<Vehicle | null> {
  ...
  async hasVehicleAccess(userId: number, vehicleId: number): Promise<boolean> {
    ...
    const userKeys = await this.digitalKeyModel.findByUserAndVehicle(
      userId,
      vehicleId,
    );
