...
class KeyService {
    ...
    async createDigitalKey(
        ...
        const existingKey = await this.digitalKeyModel.findByUserAndVehicle(userId, vehicleId);
        if (existingKey && existingKey.is_active) {
            throw new Error('Active key already exists for this user and vehicle');
        }

        const keyDataPlain = CryptoService.generateDigitalKeyData(vehicleId, userId, permissions);
    ...
    async validateDigitalKey(keyId: number, action: keyof KeyPermissions): Promise<boolean> {
        const isValid = await this.digitalKeyModel.isValidKey(keyId);
        if (!isValid) return false;

        const hasPermission = await this.digitalKeyModel.hasPermission(keyId, action);
    ...
    async executeVehicleAction(
        ...
        try {
            const isAuthorized = await this.validateDigitalKey(keyId, this.mapActionToPermission(action));
            
            if (!isAuthorized) {
                await this.logAccess(userId, vehicleId, action, 'failure', 'Unauthorized access', ipAddress, userAgent);
                return { success: false, message: 'Unauthorized: Invalid key or insufficient permissions' };
            }

            const key = await this.digitalKeyModel.findById(keyId);
            if (!key || key.user_id !== userId || key.vehicle_id !== vehicleId) {
                await this.logAccess(userId, vehicleId, action, 'failure', 'Key mismatch', ipAddress, userAgent);
            ...
            if (result.success) {
                await this.logAccess(userId, vehicleId, action, 'success', undefined, ipAddress, userAgent);
                return { success: true, message: `Vehicle ${action} successful` };
            } else {
                await this.logAccess(userId, vehicleId, action, 'failure', result.error, ipAddress, userAgent);
                return { success: false, message: result.error || 'Command execution failed' };
            }
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            await this.logAccess(userId, vehicleId, action, 'failure', errorMessage, ipAddress, userAgent);
    ...
    async verifyKeyData(keyId: number): Promise<boolean> {
        try {
            ...
            return keyInfo.vehicleId === key.vehicle_id && 
                   keyInfo.userId === key.user_id;
        } catch (error) {
            return false;
        }
    }
    ...
    async getKeyStats(userId: number): Promise<{
        ...
        const activeKeys = userKeys.filter(key => 
            key.is_active && 
        ...
        const recentAccess = recentLogs.filter(log => 
            log.result === 'success' &&
