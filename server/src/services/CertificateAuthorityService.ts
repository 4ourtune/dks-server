...
class CertificateAuthorityService {
  ...
  async initializeRootCA(): Promise<void> {
    try {
      ...
      if (existingCA) {
        const decryptedPrivateKey = this.decryptRootCAPrivateKey(
          existingCA.privateKeyEncrypted,
        );
        this.rootCAKeys = {
        ...
        this.rootCACertificate = this.buildRootCACertificate(
          existingCA.keyId,
          existingCA.createdAt,
        );
      ...
      const encryptedPrivateKey = this.encryptRootCAPrivateKey(
        keyPair.privateKey,
      );
  ...
  private encryptRootCAPrivateKey(privateKey: string): string {
    const masterPassword =
      process.env.ROOT_CA_PASSWORD || "default_ca_password";
  ...
  async issueVehicleCertificate(
    ...
    const existingCert = await this.certificateModel.findBySubjectId(
      vehicleId,
      "vehicle",
    );
  ...
  async issueDigitalKeyCertificate(
    ...
    const vehicleCert = await this.certificateModel.findBySubjectId(
      vehicleId,
      "vehicle",
    );
  ...
  async verifyCertificate(certificate: CertificateData): Promise<boolean> {
    ...
    return ECCCryptoService.verifyCertificate(
      certificate,
      this.rootCAKeys.publicKey,
    );
  }
  ...
  async revokeCertificate(
    ...
    const certificate =
      await this.certificateModel.findBySerialNumber(serialNumber);
    if (!certificate) {
  ...
  private buildRootCACertificate(
    keyId: string,
    createdAt?: string,
  ): RootCACertificate {
    if (!this.rootCAKeys) {
    ...
    const notAfter = new Date(
      issuedAt.getTime() + 10 * 365 * 24 * 60 * 60 * 1000,
    ).toISOString();
  ...
  async getRootCACertificate(): Promise<RootCACertificate> {
    ...
    if (!this.rootCACertificate) {
      this.rootCACertificate = this.buildRootCACertificate(
        this.rootCAKeys.keyId,
        this.rootCAKeys.createdAt,
      );
    }
  ...
  async getCertificateBySerialNumber(
    serialNumber: string,
  ...
  async getUserCertificates(userId: number): Promise<Certificate[]> {
    return await this.certificateModel.findAllBySubjectId(
      userId,
      "digital_key",
      true,
    );
