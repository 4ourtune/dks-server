...
class LoggerService {
    ...
    private constructor() {
        ...
        this.logDirectory = process.env.LOG_DIRECTORY || join(process.cwd(), 'logs');
    ...
    error(message: string, error?: Error | any, context?: LogContext): void {
        ...
        console.log(`${prefix} ${timeStr} ${this.colorize(message.toUpperCase(), 'bold')}`);
    ...
    // API logging
    api(method: string, path: string, statusCode: number, responseTime: number, extra?: any): void {
        if (!this.shouldLog(LogLevel.INFO)) return;
    ...
    auth(action: string, data: { email?: string; userId?: number; error?: string }): void {
        if (!this.shouldLog(LogLevel.INFO)) return;
        ...
        if (data.error) {
            ...
            console.log(`${errorPrefix} ${timeStr} Auth ${action} failed: ${data.error}`);
        } else {
    ...
    vehicle(action: string, data: { vehicleId: number; userId: number; success?: boolean; error?: string }): void {
        if (!this.shouldLog(LogLevel.INFO)) return;

        const timestamp = this.getTimestamp();
        const success = data.success !== false;
        const prefix = success ? this.colorize('[CAR]  ', 'green') : this.colorize('[ERROR]', 'red');
        const timeStr = this.colorize(`[${timestamp}]`, 'gray');
        
        const statusText = success ? 'success' : 'failed';
        const message = `Vehicle ${action}: ${statusText} (Vehicle #${data.vehicleId}, User #${data.userId})`;
        
        console.log(`${prefix} ${timeStr} ${message}${data.error ? ' - ' + data.error : ''}`);
        
        const fileContent = `[VEHICLE] [${timestamp}] ${action}: ${JSON.stringify(data)}`;
        this.writeToFile(fileContent);
    }

    key(action: string, data: { keyId: string; userId: number; vehicleId: number; success?: boolean; error?: string }): void {
        if (!this.shouldLog(LogLevel.INFO)) return;

        const timestamp = this.getTimestamp();
        const success = data.success !== false;
        const prefix = success ? this.colorize('[KEY]  ', 'yellow') : this.colorize('[ERROR]', 'red');
        const timeStr = this.colorize(`[${timestamp}]`, 'gray');
