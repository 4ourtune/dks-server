...
class ECCCryptoService extends CryptoService {
    ...
    static verifyECDSASignature(data: string, signature: string, publicKey: string): boolean {
        try {
    ...
    static createVehicleCertificate(
        ...
        const validTo = new Date(now.getTime() + validityDays * 24 * 60 * 60 * 1000).toISOString();
        ...
        certificate.signature = this.signWithECDSA(certificateData, issuerPrivateKey);
    ...
    static verifyCertificate(certificate: CertificateData, issuerPublicKey: string): boolean {
        try {
            const { signature, ...certDataWithoutSignature } = certificate;
            const certificateData = JSON.stringify(certDataWithoutSignature);

            const isValidSignature = this.verifyECDSASignature(certificateData, signature, issuerPublicKey);
            if (!isValidSignature) {
                return false;
            }
    ...
    static verifyCertificateSigningRequest(csrString: string, publicKey: string): boolean {
        try {
    ...
    static generateECDHSharedSecret(privateKey: string, otherPublicKey: string): string {
        try {
            const key = this.ec.keyFromPrivate(privateKey, 'hex');
    ...
    static deriveSessionKey(sharedSecret: string, nonce1: string, nonce2: string): string {
    ...
    static encryptWithPublicKey(data: string, publicKey: string): string {
        try {
            ...
            const sharedSecret = this.generateECDHSharedSecret(ephemeralKeyPair.privateKey, publicKey);
    ...
    static decryptWithPrivateKey(encryptedDataString: string, privateKey: string): string {
        try {
            const encryptedData = JSON.parse(encryptedDataString);
            const sharedSecret = this.generateECDHSharedSecret(privateKey, encryptedData.ephemeralPublicKey);
