  ...
  private async handleVehicleConnect(
    ...
    try {
      ...
      if (vehicle.owner_id !== socket.userId) {
        ...
        const vehicleKey = userKeys.find(
          (key) => key.vehicle_id === vehicle_id && key.is_active,
        );
      ...
      const connected = await this.vehicleService.connectVehicle(
        vehicle_id,
        device_id,
      );
  ...
  private async handleVehicleCommand(
    ...
    try {
      ...
      if (result.success) {
        ...
        const updatedStatus =
          await this.vehicleService.getVehicleStatus(vehicle_id);
  ...
  public async performPeriodicStatusUpdate(): Promise<void> {
    ...
    for (const vehicleId of connectedVehicles) {
      try {
        ...
        console.error(
          `Failed to update status for vehicle ${vehicleId}:`,
          error,
        );
      }
    }
