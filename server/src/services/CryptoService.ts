...
class CryptoService {
    ...
    static generateDigitalKeyData(vehicleId: number, userId: number, permissions: any): string {
        const keyData = {
            vehicleId,
            userId,
            permissions,
    ...
    static verifyDigitalKeyData(encryptedKeyData: string, masterKey: string, iv: string, tag: string): {
        vehicleId: number;
        userId: number;
        permissions: any;
        timestamp: number;
        ...
        try {
            ...
            const requiredFields = ['vehicleId', 'userId', 'permissions', 'timestamp', 'nonce'];
