...
export const authenticateVehicle = async (
    ...
    const secret = getHeaderValue(req.headers['x-vehicle-secret']) ?? getQueryValue(req.query.secret);
    ...
    try {
        ...
        if (!vehicle && fallbackSecret && secret === fallbackSecret) {
            ...
            if (usedFallbackSecret) {
                console.warn(`Vehicle ${vehicleId} authenticated with fallback secret. Rotate vehicle secret ASAP.`);
            }
        }
