...
export const verifyRefreshToken = (refreshToken: string): { userId: number } | null => {
    try {
        const secret = process.env.JWT_SECRET;
        if (!secret) {
            throw new Error('JWT_SECRET not configured');
...
export const requireVehicleAccess = async (req: AuthRequest, res: Response, next: NextFunction) => {
