  ...
  register = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!isOwner) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can register digital keys" });
        return;
      }
  ...
  updateKey = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!isOwner) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can modify digital keys" });
        return;
      }
  ...
  deleteKey = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!isOwner) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can delete digital keys" });
        return;
      }
