...
class AuthController {
    ...
    register = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            ...
            if (existingUser) {
                this.logger.auth('register_failed', { email, error: 'Email already exists' });
    ...
    login = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            ...
            if (!user) {
                this.logger.auth('login_failed', { email, error: 'Invalid credentials' });
    ...
    changePassword = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            ...
            const validatedUser = await this.userModel.validatePassword(user.email, currentPassword);
            if (!validatedUser) {
    ...
    deleteAccount = async (req: AuthRequest, res: Response): Promise<void> => {
        try {
            ...
            const validatedUser = await this.userModel.validatePassword(user.email, password);
            if (!validatedUser) {
            ...
            if (userVehicles.length > 0) {
                res.status(400).json({ 
                    error: 'Cannot delete account with registered vehicles. Please delete all vehicles first.' 
                });
                return;
            }
            ...
            if (activeKeys.length > 0) {
                res.status(400).json({ 
                    error: 'Cannot delete account with active digital keys. Please revoke all keys first.' 
                });
                return;
            }
