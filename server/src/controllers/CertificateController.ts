...
class CertificateController {
  ...
  private buildCertificateResponse(certificate: CertificateEntity): any {
    ...
    const notBefore =
      baseData.validFrom ?? baseData.notBefore ?? certificate.issuedAt;
    const notAfter =
      baseData.validTo ?? baseData.notAfter ?? certificate.expiresAt;
    const parsedVersion =
      typeof baseData.version === "string"
        ? parseFloat(baseData.version)
        : baseData.version;
    const version = Number.isFinite(parsedVersion)
      ? parsedVersion
      : (baseData.version ?? 1);
    ...
    if (certificate.type === "digital_key") {
      ...
      response.permissions = digitalData.permissions ?? {
        unlock: false,
        lock: false,
        startEngine: false,
      };
  ...
  issueVehicleCertificate = async (
    req: AuthRequest,
    res: Response,
  ): Promise<void> => {
    try {
      await this.caService.initializeRootCA();
      const { vehicleId, deviceSerial, manufacturer, model, validityDays } =
        req.body;
      ...
      res.status(500).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to issue vehicle certificate",
      });
    }
  };

  issueDigitalKeyCertificate = async (
    req: AuthRequest,
    res: Response,
  ): Promise<void> => {
    try {
      ...
      const normalizedPermissions: KeyPermissions = {
        ...
        startEngine: Boolean(
          (permissions as any).startEngine ??
            (permissions as any).engine_on ??
            false,
        ),
      };
      ...
      this.logger.server(
        `Digital key certificate issued for user ${userId}, vehicle ${vehicleId}`,
      );
      ...
      res.status(500).json({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to issue digital key certificate",
      });
    }
  };
  ...
  getCertificate = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const certificate =
        await this.caService.getCertificateBySerialNumber(serialNumber);
      if (!certificate) {
        res.status(404).json({
          success: false,
  ...
  getUserCertificates = async (
    ...
    try {
      ...
      const certificateResponses = certificates.map((cert) =>
        this.buildCertificateResponse(cert),
      );
  ...
  getVehicleCertificate = async (
    ...
    try {
      ...
      const certificate = await this.caService.getVehicleCertificate(
        parseInt(vehicleId),
      );
      if (!certificate) {
        res.status(404).json({
  ...
  revokeCertificate = async (
    ...
    try {
      ...
      await this.caService.revokeCertificate(
        serialNumber,
        reason || "Revoked by user",
      );

      this.logger.server(
        `Certificate ${serialNumber} revoked by user ${req.userId}`,
      );
  ...
  renewCertificate = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const newCertificate = await this.caService.renewCertificate(
        serialNumber,
        validityDays,
      );

      this.logger.server(
        `Certificate ${serialNumber} renewed by user ${req.userId}`,
      );
  ...
  getCertificateRevocationList = async (
    req: AuthRequest,
    res: Response,
  ): Promise<void> => {
    try {
  ...
  exportCertificate = async (
    ...
    try {
      ...
      if (format === "pem") {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="${serialNumber}.pem"`,
        );
      } else {
        res.setHeader("Content-Type", "application/json");
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="${serialNumber}.json"`,
        );
      }
