...
class VehicleController {
  ...
  registerSystemVehicle = async (
    ...
    try {
      ...
      const { vehicle: createdVehicle, secret } =
      ...
      this.logger.vehicle("system_register", {
  ...
  registerUserToVehicle = async (
    ...
    try {
      ...
      if (typeof pairingToken !== "string" || pairingToken.length === 0) {
        res.status(400).json({ error: "pairingToken is required" });
        return;
      }

      const result = await this.pairingService.finalizePairingSession(
        userId,
        vehicleId,
        pairingToken,
      );
      ...
      this.logger.vehicle("user_register", {
        vehicleId,
        userId,
        success: true,
      });
      ...
      res.status(200).json({
        message: "Successfully finalized vehicle pairing",
        vehicle: vehicle
          ? {
              id: vehicle.id,
              vin: vehicle.vin,
  ...
  getVehicleById = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (vehicle.owner_id !== userId) {
        ...
        const vehicleKey = hasKey.find(
          (key) => key.vehicle_id === vehicleId && key.is_active,
        );
      ...
      const isConnected =
        await this.vehicleService.isVehicleConnected(vehicleId);
  ...
  updateVehicle = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (vehicle.owner_id !== userId) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can update vehicle information" });
        return;
      }
  ...
  deleteVehicle = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (vehicle.owner_id !== userId) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can delete vehicle" });
        return;
      }
  ...
  getLogs = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (vehicle.owner_id !== userId) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can view access logs" });
        return;
      }

      const logs = await this.vehicleService.getVehicleAccessLogs(
        vehicleId,
        limit,
      );
  ...
  getStatistics = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (vehicle.owner_id !== userId) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can view statistics" });
        return;
      }
  ...
  performMaintenance = async (
    ...
    try {
      ...
      if (vehicle.owner_id !== userId) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can perform maintenance" });
        return;
      }

      const maintenanceResult =
        await this.vehicleService.performMaintenanceCheck(vehicleId);
  ...
  setMaintenanceMode = async (
    ...
    try {
      ...
      if (vehicle.owner_id !== userId) {
        res
          .status(403)
          .json({ error: "Only vehicle owner can set maintenance mode" });
        return;
      }

      const updatedVehicle =
        await this.vehicleService.setVehicleMaintenanceMode(vehicleId, enabled);
