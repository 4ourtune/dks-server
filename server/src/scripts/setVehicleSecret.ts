...
async function main(): Promise<void> {
  ...
  if (!vehicleIdArg || !secretArg) {
    console.error(
      "Usage: npm run vehicle:set-secret -- <vehicleId> <plainSecret>",
    );
    process.exitCode = 1;
    return;
  }
  ...
  try {
    const vehicle = await vehicleService.setVehicleSecret(
      vehicleId,
      plainSecret,
    );
    console.log(
      `Secret updated for vehicle ${vehicle.id} (device_id: ${vehicle.device_id}).`,
    );
  } catch (error) {
