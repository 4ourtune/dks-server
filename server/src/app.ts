...
class App {
  ...
  constructor() {
    ...
    if (process.env.NODE_ENV !== "test") {
      ...
      this.logger.server("Periodic tasks skipped in test environment");
    }
  }
      ...
      async () => {
        try {
          const CertificateAuthorityService = (
            await import("./services/CertificateAuthorityService")
          ).default;
          const caService = new CertificateAuthorityService();
          await caService.initializeRootCA();
          const cleanedCertificates =
            await caService.cleanupExpiredCertificates();

          if (cleanedCertificates > 0) {
            this.logger.server(
              `Cleaned up ${cleanedCertificates} expired certificates`,
            );
          }
        } catch (error) {
  ...
  public async close(): Promise<void> {
    try {
      ...
      console.warn("Failed to close Socket.IO server gracefully:", error);
    }
    ...
    if (this.server && typeof this.server.close === "function") {
      try {
        ...
        if ((error as any)?.code !== "ERR_SERVER_NOT_RUNNING") {
          console.warn("Failed to close HTTP server gracefully:", error);
        }
      }
    }
